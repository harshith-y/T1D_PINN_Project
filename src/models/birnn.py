...
class GRUDeCarliModel(tf.keras.Model):
    ...
    def __init__(
        ...
        # Build GRU layers
        self.gru_layers = [
            tf.keras.layers.GRU(hidden_units, return_sequences=True, name=f"gru_{i}")
            for i in range(num_layers)
        ]

        # Output layer predicts [G, I, D]
        self.output_layer = tf.keras.layers.Dense(3, activation="linear", name="output")
...
class BIRNN:
    ...
    def build(self, data_window: TrainingWindow):
        ...
        if (
            hasattr(self.config.architecture, "use_hard_ic")
            and self.config.architecture.use_hard_ic
        ):
        ...
        print(
            f"   Sequence length: {self.X_train.shape[1]} train, {self.X_test.shape[1]} test"
        )
    ...
    def _compute_biological_residual_loss(
        self, y_pred: tf.Tensor, y_in: tf.Tensor, u_in: tf.Tensor, r_in: tf.Tensor
    ) -> tf.Tensor:
        """
    ...
    def train(self, display_every: int = 50):
        ...
        if (
            hasattr(self.config.training, "stages")
            and self.config.training.stages is not None
        ):
            ...
            stages = [
                {
                    ...
                    "loss_weights": {
                        ...
                        "lambda_i": (
                            self.config.loss_weights.insulin
                            if hasattr(self.config.loss_weights, "insulin")
                            else 0.0
                        ),
                        "lambda_d": (
                            self.config.loss_weights.digestion
                            if hasattr(self.config.loss_weights, "digestion")
                            else 0.0
                        ),
                        "lambda_ic": (
                            self.config.loss_weights.ic
                            if hasattr(self.config.loss_weights, "ic")
                            else 0.0
                        ),
        ...
        for stage_idx, stage in enumerate(stages):
            ...
            print(
                f"    Loss weights: G={lambda_g:.2f}, I={lambda_i:.2f}, D={lambda_d:.2f}, IC={lambda_ic:.2f}, Bio={lambda_B:.2f}"
            )
            ...
            for epoch in range(epochs):
                ...
                if epoch % display_every == 0:
                    tf.print(
                        f"[Stage {stage_idx+1} | Epoch {epoch}]",
                        "Total=",
                        loss,
                        ", G=",
                        lg,
    ...
    def predict(self, use_test_set: bool = False) -> Dict[str, np.ndarray]:
        ...
        return {"G": G_pred, "I": I_pred, "D": D_pred, "t_min": t_min}
