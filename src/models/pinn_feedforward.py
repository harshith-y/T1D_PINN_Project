...
class FeedforwardPINN:
        ...
        if config.mode == "inverse":
            ...
            try:
                ...
                for param_name in inverse_params_list:
                    ...
                    estimated_value = self.inverse_params_obj.get_param_value(
                        param_name, as_float=True
                    )
    ...
    def build(self, data_window: TrainingWindow) -> None:
        ...
        layers = (
            [1]
            + [self.config.architecture.n_neurons] * self.config.architecture.n_layers
            + [3]
        )
        self.net = dde.nn.FNN(
            layers, self.config.architecture.activation, "Glorot uniform"
        )
    ...
    def _apply_fourier_transform(self) -> None:
        ...
        print(
            f"   Applied Fourier transform with features: {self.config.architecture.fourier_features}"
        )
    ...
    def _apply_hard_initial_conditions(self) -> None:
        ...
        I0 = (
            self.data_window.insulin[0] / self.scales.m_i
            if self.data_window.insulin is not None
            else 0.0
        )
        D0 = (
            self.data_window.digestion[0] / self.scales.m_d
            if self.data_window.digestion is not None
            else 0.0
        )
    ...
    def _prepare_training_data(self) -> tuple[np.ndarray, np.ndarray]:
        ...
        print(
            f"   Training points: {len(t_train)} (from {len(self.data_window.time_norm)} total)"
        )
