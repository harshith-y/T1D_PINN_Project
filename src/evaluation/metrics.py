...
def compute_glucose_metrics(
    pred: np.ndarray, true: np.ndarray, split_idx: Optional[int] = None
) -> Dict[str, float]:
    """
...
def compute_latent_metrics(
    ...
    if insulin_pred is not None and insulin_true is not None:
        metrics["insulin_rmse"] = float(
            np.sqrt(np.mean((insulin_pred - insulin_true) ** 2))
        )
        ...
        if split_idx:
            ...
            metrics["insulin_rmse_test"] = float(
                np.sqrt(np.mean((test_pred - test_true) ** 2))
            )

    # Digestion metrics
    if digestion_pred is not None and digestion_true is not None:
        metrics["digestion_rmse"] = float(
            np.sqrt(np.mean((digestion_pred - digestion_true) ** 2))
        )
        metrics["digestion_mae"] = float(
            np.mean(np.abs(digestion_pred - digestion_true))
        )

        if split_idx:
            test_pred = digestion_pred[split_idx:]
            test_true = digestion_true[split_idx:]
            metrics["digestion_rmse_test"] = float(
                np.sqrt(np.mean((test_pred - test_true) ** 2))
            )
