...
class ExperimentPlotter:
    ...
    def __init__(
        ...
        self.style = MODEL_STYLES.get(
            model_name, {"color": "blue", "linestyle": "--", "linewidth": 2}
        )
    ...
    def plot_latent_variables(
        ...
        fig, (ax1, ax2) = plt.subplots(
            2, 1, figsize=(self.figsize_timeseries[0], 10), sharex=True
        )
    ...
    def plot_rmse_comparison(
        self, rmse_data: Dict[str, float], save_name: str = "rmse_comparison.png"
    ) -> Path:
        """
        ...
        bars = ax.bar(
            variables, values, color=["#1f77b4", "#ff7f0e", "#2ca02c"], alpha=0.7
        )
    ...
    def plot_patient_data_only(
        ...
        axes[plot_idx].plot(
            time, glucose, linewidth=2, color="#1f77b4", label="Glucose"
        )
        axes[plot_idx].set_ylabel("Glucose (mg/dL)", fontsize=12)
        axes[plot_idx].set_title(
            f"{patient_id} - Data Profile", fontsize=14, fontweight="bold"
        )
        ...
        if insulin is not None:
            axes[plot_idx].plot(
                time, insulin, linewidth=2, color="#2ca02c", label="Insulin"
            )
        ...
        if digestion is not None:
            axes[plot_idx].plot(
                time, digestion, linewidth=2, color="#ff7f0e", label="Digestion"
            )
        ...
        if u is not None or r is not None:
            ...
            ax.set_ylabel(
                "Insulin u(t) (U/min)",
                fontsize=12,
                color="#d62728" if u is not None else "black",
            )
            if u is not None:
    ...
    def plot_all(
        ...
        if "glucose" in predictions and "time" in predictions:
            ...
            path = self.plot_residuals(
                time=predictions["time"], residuals=residuals, split_idx=split_idx
            )
...
def plot_multi_model_glucose(
    ...
    # Plot ground truth
    ax.plot(
        time, ground_truth["glucose"], label="True Glucose", color="blue", linewidth=2
    )

    # Plot each model
    for model_name, preds in results.items():
        style = MODEL_STYLES.get(
            model_name, {"color": "gray", "linestyle": "--", "linewidth": 2}
        )
        ax.plot(time, preds["glucose"], label=f"{model_name}", **style)
...
def create_results_summary_table(
    results: Dict[str, Dict[str, Any]], save_path: str | Path = "results_summary.csv"
) -> Path:
    """
...
""",
    ...
    if args.real_patient is not None:
        ...
        try:
            ...
            # Optional columns
            insulin = (
                df["insulin"].values.astype(np.float32)
                if "insulin" in df.columns
                else None
            )
            digestion = (
                df["carbohydrates"].values.astype(np.float32)
            ...
            class SimpleData:
                def __init__(self):
                    ...
                    self.has_latent_states = (
                        insulin is not None and digestion is not None
                    )
            ...
            if args.data_only:
                ...
                predictions = {
                    "time": time,
                    "glucose": data.glucose
                    + np.random.randn(len(time)) * 3,  # Add small noise
                }
                ...
                metrics = {"rmse_glucose": 18.5, "rmse_total": 19.2}
    ...
    if args.patient is not None:
        ...
        try:
            ...
            try:
                ...
                print(
                    f"   - Does src/datasets/ exist? {(project_root / 'src' / 'datasets').exists()}"
                )
                print("   Falling back to dummy data...\n")
                args.patient = None
            ...
            if args.patient is not None:
                ...
                predictions = {
                    "time": time,
                    "glucose": data.glucose
                    + np.random.randn(len(time)) * 2,  # Add small noise
                    "insulin": data.insulin,
                    "digestion": data.digestion,
                }
                ground_truth = {
    ...
    if args.patient is None and args.real_patient is None:
        ...
        y_true = (
            100 + 20 * np.sin(2 * np.pi * time / 1440) + np.random.randn(len(time)) * 5
        )
        ...
        if args.data_only:
            ...
            predictions = {"time": time, "glucose": y_pred}
            ground_truth = {"glucose": y_true}
            metrics = {"rmse_glucose": 10.5, "rmse_total": 12.3}
            param_history = None
            true_param = None
    ...
    if not skip_predictions:
        ...
        plotter = ExperimentPlotter(save_dir=args.save_dir, model_name=args.model_name)
