...
class Config:
    ...
    def validate(self) -> None:
        ...
        if self.model_name == 'modified_mlp' and self.architecture.use_fourier:
            print("⚠️  Warning: Fourier features not recommended for Modified MLP (may degrade performance)")
        
        if self.model_name == 'birnn' and self.architecture.use_fourier:
            print("⚠️  Warning: Fourier features not applicable to BI-RNN (will be ignored)")
        ...
        if self.mode == 'inverse':
            ...
            if self.inverse_param not in valid_inverse_params:
                raise ValueError(f"Invalid inverse_param: {self.inverse_param}. Must be one of {valid_inverse_params}")
