...
class UnifiedTrainer:
    ...
    def _train_stage_deepxde(self, stage: TrainingStage) -> None:
        ...
        if stage.train_inverse_params:
            if self.model.inverse_params and self.model.inverse_params.log_ksi is not None:
        ...
        if external_vars:
            ...
            param_logger = dde.callbacks.VariableValue(
                external_vars,
                period=stage.display_every,
                filename=str(param_log_path)
            )
    ...
    def _train_stage_birnn(self, stage: TrainingStage) -> None:
        ...
        self.model.lambda_g = stage.loss_weights[0] if len(stage.loss_weights) > 0 else 8.0
        self.model.lambda_B = stage.loss_weights[1] if len(stage.loss_weights) > 1 else 4.82
        self.model.lambda_ic = stage.loss_weights[2] if len(stage.loss_weights) > 2 else 0.53
        ...
        for epoch in range(stage.epochs):
            ...
            if epoch % stage.display_every == 0:
                print(f"Epoch {epoch}/{stage.epochs}: " + 
        ...
        self.history['losses'].append({
            'stage': stage.name,
            'final_losses': loss_dict
        })
    
    def _birnn_train_step_with_varlist(self, var_list: List[tf.Variable]) -> Dict[str, float]:
        """
        ...
        with tf.GradientTape() as tape:
            ...
            loss_g = tf.reduce_mean(tf.square(Y_pred[:, :, 0:1] - self.model.Y_train[:, :, 0:1]))
            ...
            if self.model.lambda_ic > 0:
                loss_ic = tf.reduce_mean(tf.square(Y_pred[:, 0, :] - self.model.Y_train[:, 0, :]))
            else:
                loss_ic = 0.0
            
            # Total loss
            total_loss = (self.model.lambda_g * loss_g + 
    ...
    def _log_inverse_parameters(self) -> None:
        ...
        if self.model_type in ['pinn', 'modified_mlp']:
            ...
            if hasattr(self.model, 'log_ksi') and self.model.log_ksi is not None:
                ...
                try:
                    patient_num = int(self.model.data_window.patient_id.replace("Pat", ""))
