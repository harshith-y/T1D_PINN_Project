...
class InverseTrainer:
    ...
    def __init__(
        ...
        self.param_history = {"epochs": [], "stages": []}
    ...
    def train(self) -> Dict[str, Any]:
        ...
        for i, stage_dict in enumerate(self.config.training.stages, 1):
            ...
            cumulative_epoch = self._train_stage(
                stage_dict, cumulative_epoch, stage_name=stage_dict["name"]
            )
        ...
        for param_name in self.inverse_params_list:
            ...
            if param_name == self.inverse_params_list[0] and self.true_param_value:
                error = (
                    abs(final_value - self.true_param_value)
                    / self.true_param_value
                    * 100
                )
    ...
    def _train_stage(
        self, stage_config: Dict, cumulative_epoch: int, stage_name: str
    ) -> int:
        """
        ...
        for epoch in range(epochs):
            ...
            if epoch % 10 == 0:
                ...
                for param_name in self.inverse_params_list:
                    ...
                    self.param_history[f"{param_name}_losses"].append(
                        loss_dict["total"]
                    )
    ...
    def _train_stage_deepxde(
        ...
        print(
            f"        Inverse params ({len(inverse_param_vars)} vars): {'TRAINING' if train_params else 'FROZEN'}"
        )
        print(
            f"        NN weights ({len(nn_weight_vars)} vars): {'TRAINING' if train_nn else 'FROZEN'}"
        )
        ...
        class ParameterTracker(dde.callbacks.Callback):
            ...
            def on_epoch_end(self):
                ...
                for param_name in self.param_names:
                    ...
                    log_param_var = getattr(
                        self.trainer.model.inverse_params_obj, f"log_{param_name}", None
                    )
                    ...
                    try:
                        ...
                        if hasattr(self.model, "sess"):
                            ...
                            param_val = (
                                self.trainer.model.inverse_params_obj.get_param_value(
                    ...
                    if hasattr(self.model.train_state, "loss_train"):
                        ...
                        if isinstance(loss_array, (list, tuple)) or hasattr(
                            loss_array, "__len__"
                        ):
        ...
        for param_name in self.inverse_params_list:
            ...
            if self.param_history[f"{param_name}_values"]:
                ...
                if (
                    param_name == self.inverse_params_list[0]
                    and self.true_param_value is not None
                ):
                    error = (
                        abs(param_value - self.true_param_value)
                        / self.true_param_value
                        * 100
                    )
    ...
    def _get_trainable_vars(
        self, train_params: bool, train_nn: bool
    ) -> List[tf.Variable]:
        """
    ...
    def _compute_biological_residual(self, Y_pred: tf.Tensor) -> tf.Tensor:
        ...
        ku_Vi = get_param_value(
            self.model.inverse_params_obj, self.model.params, "ku_Vi"
        )
        ...
        kr_Vb = get_param_value(
            self.model.inverse_params_obj, self.model.params, "kr_Vb"
        )
    ...
    def _train_step_deepxde(
        self, var_list: list, training: bool, stage_config: dict, optimizer
    ) -> dict:
        """
    ...
    def _get_current_param_value(self, param_name: str = "ksi") -> float:
        ...
        if (
            hasattr(self.model, "inverse_params_obj")
            and self.model.inverse_params_obj is not None
        ):
            ...
            if value is not None:
                ...
                if isinstance(value, tf.Tensor) or isinstance(value, tf.Variable):
                    ...
                    try:
                        ...
                        print(
                            f"⚠️  Warning: Could not extract value for {param_name} from TF graph"
                        )
                        return float("nan")
                return float(value)
