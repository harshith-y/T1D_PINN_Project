...
def compute_bio_residual_corrected(Y_pred, Y_in, U_in, R_in, log_ksi, params, u_max, r_max, m_g, m_i, m_d, m_t):
    """
...
for epoch in range(50):
    with tf.GradientTape() as tape:
        ...
        loss_B = compute_bio_residual_corrected(
            Y_pred, model.Y_train, model.U_train, model.R_train,
            model.log_ksi, model.params,
            model.u_max, model.r_max,  # Stored max values
            model.data_window.m_g, model.data_window.m_i, model.data_window.m_d,
    ...
    if epoch == 0:
        if gradients[0] is None:
            ...
            print(f"✅ Gradient check: grad magnitude = {float(tf.abs(gradients[0])):.6f}")
    ...
    if epoch % 10 == 0:
        ...
        print(f"Epoch {epoch:3d}: loss_g={float(loss_g):.6f}, loss_B={float(loss_B):.6f}, ksi={ksi_val:.2f} (error={error:.2f}%)")
...
for epoch in range(50):
    ...
    if epoch % 20 == 0:
        print(f"Epoch {epoch:3d}: total={float(total_loss):.4f}, loss_g={float(loss_g):.4f}, loss_B={float(loss_B):.4f}")
...
for epoch in range(50):
    ...
    if epoch % 10 == 0:
        ksi_val = float(tf.exp(model.log_ksi))
        error = abs(ksi_val - true_ksi) / true_ksi * 100
        print(f"Epoch {epoch:3d}: total={float(total_loss):.4f}, ksi={ksi_val:.2f} (error={error:.2f}%)")
...
test_loss_g = tf.reduce_mean(tf.square(Y_pred_test[:, :, 0:1] - model.Y_test[:, :, 0:1]))
