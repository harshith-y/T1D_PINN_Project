...
def test_simulator_output():
    ...
    expected_cols = ['time', 'patient', 'glucose', 'glucose_derivative', 
                     'insulin', 'insulin_derivative', 'carbohydrates', 'u', 'r']
...
def test_loader_basic():
    ...
    assert np.isclose(window.time_norm[-1], 1.0, atol=1e-3), "Normalized time should end at ~1.0"
...
def test_scaling_calculations(window: TrainingWindow):
    ...
    assert np.allclose(window.time_norm, expected_time_norm), "time_norm calculation error"
...
def test_multiple_patients():
    ...
    for pat in [2, 4]:
        ...
        # Load
        window = load_synthetic_window(patient=pat, root=project_root / "data" / "synthetic")
...
def test_time_slicing():
    ...
    # Load full window
    window_full = load_synthetic_window(patient=3, root=project_root / "data" / "synthetic", 
                                       t_start=0, t_end=2880)
    
    # Load partial window (first 24 hours)
    window_24h = load_synthetic_window(patient=3, root=project_root / "data" / "synthetic",
                                      t_start=0, t_end=1440)
    
    print(f"  Full window length: {len(window_full.glucose)}")
    print(f"  24h window length: {len(window_24h.glucose)}")
    
    assert len(window_24h.glucose) < len(window_full.glucose), "Sliced window should be shorter"
    assert len(window_24h.glucose) == 1441, f"24h should be 1441 minutes, got {len(window_24h.glucose)}"
    
    # Verify first values match
    assert np.allclose(window_full.glucose[:100], window_24h.glucose[:100]), \
