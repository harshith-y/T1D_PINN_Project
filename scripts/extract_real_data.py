...
def extract_patient_data(
    user_id: str, data_df: pd.DataFrame, output_dir: Path, verbose: bool = True
) -> bool:
    """
    ...
    basal_data["duration_min"] = (
        basal_data["end_date"] - basal_data["start_date"]
    ).dt.total_seconds() / 60
    ...
    basal_df = (
        pd.DataFrame(basal_expanded)
        if basal_expanded
        else pd.DataFrame(columns=["time_min", "ut_basal"])
    )
    ...
    if not glucose_data.empty:
        ...
        glucose_df = (
            glucose_data.groupby("time_min").tail(1)[["time_min", "value"]].copy()
        )
    ...
    if not basal_df.empty:
        combined = combined.merge(
            basal_df.groupby("time_min").sum().reset_index(), on="time_min", how="left"
        )
    else:
    ...
    if not bolus_df.empty:
        combined = combined.merge(
            bolus_df.groupby("time_min").sum().reset_index(), on="time_min", how="left"
        )
    else:
    ...
    if not cho_df.empty:
        combined = combined.merge(
            cho_df.groupby("time_min").sum().reset_index(), on="time_min", how="left"
        )
    else:
        combined["rt"] = 0.0
    ...
    if verbose:
        ...
        print(
            f"    Duration: {duration_hours:.1f} hours, Glucose readings: {n_glucose}"
        )
...
def main():
    ...
    if args.patients:
        ...
        if len(patients_to_process) < len(args.patients):
            print(
                f"  ⚠️  Warning: {len(args.patients) - len(patients_to_process)} patient IDs not found"
            )
    else:
    ...
    for i, user_id in enumerate(patients_to_process, 1):
        ...
        success = extract_patient_data(
            user_id, data_df, output_dir, verbose=args.verbose
        )
        if success:
            success_count += 1
    ...
    print("=" * 80)
    print(
        f"Successfully processed: {success_count}/{len(patients_to_process)} patients"
    )
    if success_count > 0:
