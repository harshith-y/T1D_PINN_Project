...
def train_with_visualization(
    ...
    if save_dir is None:
        ...
        patient_label = (
            f"Pat{patient}" if data_type == "synthetic" else f"RealPat{patient}"
        )
    ...
    if data_type == "synthetic":
        ...
        print(
            f"   ⚠️  Limited to first 48 hours (2880 minutes) to prevent memory issues"
        )
    else:
    ...
    print(
        f"   Glucose range: {data.glucose.min():.1f} - {data.glucose.max():.1f} mg/dL"
    )
    ...
    if model_name == "birnn":
        ...
        print(
            f"   Glucose pred range: {pred_glucose.min():.1f} - {pred_glucose.max():.1f} mg/dL"
        )
        print(
            f"   Glucose true range: {true_glucose.min():.1f} - {true_glucose.max():.1f} mg/dL"
        )
    ...
    if pred_insulin is not None:
        ...
        rmse_total = float(
            np.sqrt(rmse_glucose**2 + rmse_insulin**2 + rmse_digestion**2)
        )
    else:
    ...
    plotter = ExperimentPlotter(
        save_dir=save_dir, model_name=f"{model_display} Forward"
    )
    ...
    ground_truth_dict = {"glucose": true_glucose}

    metrics_dict = {"rmse_glucose": rmse_glucose, "rmse_total": rmse_total}
