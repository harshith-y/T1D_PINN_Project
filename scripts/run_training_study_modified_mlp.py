#!/usr/bin/env python3
"""
Rigorous Training Study: Compare 8 approaches for Modified-MLP inverse parameter estimation.

This script runs a comprehensive comparison of training approaches for the Modified-MLP
model (using DeepXDE) on the inverse problem (ksi estimation + glucose forecasting).

Usage:
    python scripts/run_training_study_modified_mlp.py [--pilot] [--approach APPROACH] [--patient PAT]
...
# =============================================================================
# APPROACH CONFIGURATIONS FOR MODIFIED-MLP
# =============================================================================

# Base architecture for Modified-MLP (matching modified_mlp_inverse.yaml)
BASE_ARCHITECTURE = {
    "n_layers": 3,
    "n_neurons": 30,
    "activation": "tanh",
    ...
    "use_hard_ic": True,  # Modified-MLP uses hard initial conditions
}

# Loss weights for Modified-MLP: [glucose_obs, ode_glucose, ode_insulin, ode_digestion]
# Standard values from modified_mlp_inverse.yaml
MLOSS_WEIGHTS_STANDARD = [5.97, 4.70, 2.71, 3.32]
MLOSS_WEIGHTS_LOW_PHYSICS = [5.97, 0.5, 0.5, 0.5]
MLOSS_WEIGHTS_MEDIUM_PHYSICS = [5.97, 2.0, 1.5, 1.5]
MLOSS_WEIGHTS_HIGH_PHYSICS = [5.97, 8.0, 5.0, 5.0]
MLOSS_WEIGHTS_GLUCOSE_ONLY = [5.97, 0.0, 0.0, 0.0]
...
def get_approach_configs() -> Dict[str, List[Dict[str, Any]]]:
    """Return stage configurations for all 8 approaches adapted for Modified-MLP."""
    ...
    approaches["broken_baseline"] = [
        {
            ...
            "loss_weights": MLOSS_WEIGHTS_GLUCOSE_ONLY,  # BUG: No physics!
        ...
        },
        {
            ...
            "loss_weights": MLOSS_WEIGHTS_STANDARD,
    ...
    approaches["fixed_curriculum"] = [
        {
            ...
            "epochs": 3000,
            "learning_rate": LR_HIGH,
            "loss_weights": MLOSS_WEIGHTS_GLUCOSE_ONLY,  # Glucose only
            "train_inverse_params": False,
            "train_nn_weights": True,
            "optimizer": "adam",
            "display_every": 500,
        },
        {
            ...
            "epochs": 3000,
            "learning_rate": LR_HIGH,
            "loss_weights": MLOSS_WEIGHTS_STANDARD,  # Physics enabled
            "train_inverse_params": True,
            "train_nn_weights": False,
            "optimizer": "adam",
            "display_every": 500,
    ...
    approaches["gradual_physics"] = [
        {
            ...
            "epochs": 2000,
            "learning_rate": LR_HIGH,
            "loss_weights": MLOSS_WEIGHTS_LOW_PHYSICS,  # Very low physics
            "train_inverse_params": True,
            "train_nn_weights": True,
            "optimizer": "adam",
            "display_every": 500,
        },
        {
            ...
            "epochs": 2000,
            "learning_rate": LR_MED,
            "loss_weights": MLOSS_WEIGHTS_MEDIUM_PHYSICS,  # Medium physics
            "train_inverse_params": True,
            "train_nn_weights": True,
            "optimizer": "adam",
            "display_every": 500,
        },
        {
            ...
            "epochs": 2000,
            "learning_rate": LR_LOW,
            "loss_weights": MLOSS_WEIGHTS_STANDARD,  # Standard physics
            "train_inverse_params": True,
            "train_nn_weights": True,
            "optimizer": "adam",
            "display_every": 500,
        },
        {
            ...
            "epochs": 1000,
            "learning_rate": LR_LOW,
            "loss_weights": MLOSS_WEIGHTS_HIGH_PHYSICS,  # High physics
            "train_inverse_params": True,
            "train_nn_weights": True,
            "optimizer": "adam",
            "display_every": 500,
    ...
    approaches["physics_heavy_start"] = [
        ...
        },
        {
            ...
            "epochs": 3000,
            "learning_rate": LR_MED,
            "loss_weights": MLOSS_WEIGHTS_STANDARD,  # Standard
            "train_inverse_params": True,
            "train_nn_weights": True,
            "optimizer": "adam",
            "display_every": 500,
        },
        {
            ...
            "epochs": 1000,
            "learning_rate": LR_LOW,
            "loss_weights": MLOSS_WEIGHTS_GLUCOSE_ONLY,  # No physics, pure data fit
            "train_inverse_params": False,
            "train_nn_weights": True,
            "optimizer": "adam",
            "display_every": 500,
    ...
    approaches["warm_start_ksi"] = [
        {
            ...
            "epochs": 2000,
            "learning_rate": LR_HIGH,
            "loss_weights": MLOSS_WEIGHTS_GLUCOSE_ONLY,  # No physics initially
            "train_inverse_params": False,
            "train_nn_weights": True,
            "optimizer": "adam",
            "display_every": 500,
        },
        {
            ...
            "epochs": 2000,
            "learning_rate": LR_MED,
            "loss_weights": MLOSS_WEIGHTS_MEDIUM_PHYSICS,  # Gentle physics
            "train_inverse_params": True,
            "train_nn_weights": True,
            "optimizer": "adam",
            "display_every": 500,
        ...
        },
        {
            ...
            "epochs": 1000,
            "learning_rate": LR_LOW,
            "loss_weights": MLOSS_WEIGHTS_HIGH_PHYSICS,
            "train_inverse_params": True,
            "train_nn_weights": True,
            "optimizer": "adam",
            "display_every": 500,
...
def run_single_experiment(
    ...
    """Run a single training experiment for Modified-MLP."""
    ...
    config_dict = {
        ...
        "training": {
            ...
            "n_collocation": 3000,  # Collocation points for ODE residuals
        },
        "data": {
        ...
        "loss_weights": {
            "glucose": MLOSS_WEIGHTS_STANDARD[0],
            "ode_glucose": MLOSS_WEIGHTS_STANDARD[1],
            "ode_insulin": MLOSS_WEIGHTS_STANDARD[2],
            "ode_digestion": MLOSS_WEIGHTS_STANDARD[3],
        },
        "inverse_param": "ksi",
    ...
    # Build model - using ModifiedMLPPINN
    model = ModifiedMLPPINN(cfg)
    model.build(data)
    model.compile()

    # Get initial ksi value (for Modified-MLP, we need to run a session to get the value)
...
def run_study(
    ...
    output_dir = Path(f"results/training_study_modified_mlp_{timestamp}")
    ...
    print(f"\n{'='*70}")
    print(f"MODIFIED-MLP STUDY COMPLETE")
...
def generate_comprehensive_report(output_dir: Path) -> None:
    ...
    report_lines = []
    report_lines.append(
        "# Modified-MLP Training Approach Comparison Study - Comprehensive Report\n"
    )
    ...
    report_lines.append(f"- **Model**: Modified-MLP (DeepXDE, gated U-V encoding)\n")
...
def upload_results_to_s3(
    ...
    try:
        ...
        s3_path = f"s3://{bucket}/training_study_modified_mlp/{output_dir.name}/"
...
def main():
    parser = argparse.ArgumentParser(
        description="Run Modified-MLP training approach comparison study"
    )
    parser.add_argument(
